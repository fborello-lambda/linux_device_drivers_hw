...
set_overlay() {
    ...
    if sudo grep -Eq "^[[:space:]]*#.*uboot_overlay_addr[0-9]+=.*${dtbo}" "$uenv"; then
        # use portable POSIX character class instead of \s and avoid \+ (not portable)
        sudo sed -i "s|^[[:space:]]*#[[:space:]]*\(uboot_overlay_addr[0-9][0-9]*=.*${dtbo}\)|\1|" "$uenv"
...
mount_module() {
    if [ "$#" -ne 1 ]; then
        echo "Usage: mount_module <0|1>  # 0=normal, 1=background"
        return 1
    fi
...
case "${1:-}" in
    mount) mount_module 0 ;;
    mountb) mount_module 1 ;;
    clean) clean_module ;;
    set_overlay) set_overlay ;;
    *) echo "Usage: $0 {mount|mountb|clean|set_overlay}"; exit 1 ;;
esac

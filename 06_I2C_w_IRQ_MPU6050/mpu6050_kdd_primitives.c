...
static int mpu6050_write_reg(mpu6050_t *dev, __u8 reg, __u8 val)
{
    ...
    int ret = i2c_smbus_write_byte_data(dev->client, reg, val);
    return (ret < 0) ? ret : MPU6050_OK;
}

int mpu6050_kdd_init(mpu6050_t *dev, mpu6050_config_full_t cfg, struct i2c_client *client)
{
    if (!dev || !client)
    ...
    {
        int __st = i2c_smbus_read_byte_data(dev->client, MPU6050_REG_INT_STATUS);
        TO_BIN(__st, bin);
        if (__st >= 0)
...
static int mpu6050_kdd_get_fifo_count(mpu6050_t *dev, unsigned int *count)
{
    ...
    hi = i2c_smbus_read_byte_data(dev->client, MPU6050_REG_FIFO_COUNTH);
    if (hi < 0)
        return hi;
    lo = i2c_smbus_read_byte_data(dev->client, MPU6050_REG_FIFO_COUNTL);
    if (lo < 0)
        return lo;
                                  ...
                                  size_t max_samples)
{
    ...
    pr_debug("MPU6050: %lu frames converted to samples\n", frames);
...
int mpu6050_kdd_whoami(mpu6050_t *dev, __u8 *whoami)
{
    ...
    int ret = i2c_smbus_read_byte_data(dev->client, MPU6050_REG_WHO_AM_I);
    if (ret < 0)
        return MPU6050_ERR;
...
int mpu6050_kdd_reset(mpu6050_t *dev)
{
    ...
    int ret = i2c_smbus_write_byte_data(dev->client, MPU6050_REG_PWR_MGMT_1, 0b10000000);
    if (ret < 0)
        return MPU6050_ERR;
